library(dplyr)
library(readr)
setwd("/Volumes/Data/Alam/Web_Scraping/Scrapy/Todos_medios/tvazteca")

#Definir directorio de archivos
files <- list.files(path="/Volumes/Data/Alam/Web_Scraping/Scrapy/Todos_medios/tvazteca")
lista2<-matrix(files)
...
for (j in 1:narchivos){ 
  ...
  fecha_ws<-substr(name,11,16)
  ano<-substr(name,11,12)
  mes<-substr(name,13,14)
  dia<-substr(name,15,16)
  hora<-substr(name,18,21)
  periodico<-substr(name,4,9)
  ...
  print(j)
  name2<-paste("TvaztecaNuevoEsq",fecha_ws,hora,sep="-",".csv")
