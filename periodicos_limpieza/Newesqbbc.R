library(dplyr)
library(readr)
setwd("/Volumes/Data/Alam/Web_Scraping/Scrapy/Todos_medios/bbc")

#Definir directorio de archivos
files <- list.files(path="/Volumes/Data/Alam/Web_Scraping/Scrapy/Todos_medios/bbc")
lista2<-matrix(files)
...
for (j in 1:narchivos){ 
  ...
  print(j)
  name2<-paste("BbcNuevoEsq",fecha_ws,hora,sep="-",".csv")
