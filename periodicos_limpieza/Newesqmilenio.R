library(dplyr)
library(readr)
setwd("/Volumes/Data/Alam/Web_Scraping/Scrapy/Todos_medios/milenio")

#Definir directorio de archivos
files <- list.files(path="/Volumes/Data/Alam/Web_Scraping/Scrapy/Todos_medios/milenio")
lista2<-matrix(files)
...
for (j in 1:narchivos){ 
  ...
  print(j)
  name2<-paste("MilenioNuevoEsq",fecha_ws,hora,sep="-",".csv")
