library(dplyr)
library(readr)
setwd("/Volumes/Data/Alam/Web_Scraping/Scrapy/Todos_medios/independet")

#Definir directorio de archivos
files <- list.files(path="/Volumes/Data/Alam/Web_Scraping/Scrapy/Todos_medios/independet")
lista2<-matrix(files)
...
for (j in 1:narchivos){ 
  ...
  fecha_ws<-substr(name,15,20)
  ano<-substr(name,15,16)
  mes<-substr(name,17,18)
  dia<-substr(name,19,20)
  hora<-substr(name,22,25)
  periodico<-substr(name,4,13)
  ...
  print(j)
  name2<-paste("IndependetNuevoEsq",fecha_ws,hora,sep="-",".csv")
