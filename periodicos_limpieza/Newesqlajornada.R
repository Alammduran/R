library(dplyr)
library(readr)
setwd("/Volumes/Data/Alam/Web_Scraping/Scrapy/Todos_medios/lajornada")

#Definir directorio de archivos
files <- list.files(path="/Volumes/Data/Alam/Web_Scraping/Scrapy/Todos_medios/lajornada")
lista2<-matrix(files)
...
for (j in 1:narchivos){ 
  ...
  print(j)
  name2<-paste("LajornadaNuevoEsq",fecha_ws,hora,sep="-",".csv")
...
write.csv(todos, file = "Todos_medios.csv", enconding = "ISO-8859-1")
